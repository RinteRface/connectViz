...
rsc_users <- get_users(rsc_client, limit = Inf)
...
```{r}
create_app_daily_usage(apps_ranking[[2]])
```

#### Cumulated duration / user

```{r duration-plot}
create_cumulated_duration_per_user(apps_ranking[[1]])
```

#### Number Hits / user

```{r sessions-plot}
create_cumulated_hits_per_user(apps_ranking[[1]])
```
...
create_dev_ranking_chart(developers_apps_ranking)
```

#### Developer projects overview

```{r}
create_dev_project_overview(developers_apps_ranking, rsc_client, apps_ranking[[1]])
```
...
sort_users_by_role(rsc_users) %>% create_pie_chart()
```
...
sort_content_by_access(rsc_content) %>% create_pie_chart()
```
...
```{r content-r-version}
sort_content_by_rversion(rsc_content) %>% create_pie_chart()
```

#### Content type

```{r content-type}
sort_content_by_appmode(rsc_content) %>% create_pie_chart()
```
