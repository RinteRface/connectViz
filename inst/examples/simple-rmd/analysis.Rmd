...
create_app_daily_usage(apps_ranking[[2]], input$selected_app)
```

#### Cumulated duration / user

```{r duration-plot}
create_cumulated_duration_per_user(apps_ranking[[1]], input$selected_app)
```

#### Number Hits / user

```{r sessions-plot}
create_cumulated_hits_per_user(apps_ranking[[1]], input$selected_app)
```
...
create_dev_ranking_chart(developers_apps_ranking, input$apps_threshold)
```
...
create_dev_project_overview(developers_apps_ranking, rsc_client, apps_ranking[[1]], input$app_developer)
```
